...
class _MarmottantEquation:
    """Mixin providing the Marmottant bubble equation with vdW gas and compressibility.

    Expects the concrete class to supply the fields: ``R0``, ``P_gas0``,
    ``gamma``, ``mu_L``, ``kappa_s``, ``rho_L``, ``c_L``, ``P_amb``,
    ``vdw_divisor``, and a ``surface_tension(R)`` method.
    """
...
class Marmottant(_MarmottantEquation, Bubble):
    """
    Marmottant shell model for encapsulated microbubbles.
