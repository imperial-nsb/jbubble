...
# Create pulses
pulse_stable = ToneBurst(
    freq=1e6, pressure=low_pressure, shape=Sine(), cycle_num=5, envelope=HannEnvelope()
)
pulse_inertial = ToneBurst(
    freq=1e6, pressure=high_pressure, shape=Sine(), cycle_num=5, envelope=HannEnvelope()
)
...
results = v_sim(
    jax.tree.map(lambda x, y: jax.numpy.stack([x, y]), pulse_stable, pulse_inertial)
)
