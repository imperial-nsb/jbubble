...
bubble_circle = plt.Circle(
    (0, 0), bubble_r[0], color="tab:blue", alpha=0.6, label="Bubble"
)
vessel_circle = plt.Circle(
    (0, 0), vessel_r[0], fill=False, edgecolor="tab:red", linewidth=2.5, label="Vessel"
)
...
def spherical_confinement(freq, r0):
    bubble = SphericalConfinement(
        R0=r0, vessel_radius=10e-6, vessel_E=0.1e6
    )  # Fixed vessel radius for sweep
...
def plot_heatmaps(freq_grid, r0_grid, data):
...
plot_heatmaps(freq_grid, r0_grid, data_spherical)
